...
> (그래프1) Netflix 가입자 증가 / (그래프2) 한국 OTT 이용률 / (그래프3) Time Spent Watching(L) Choosing(R)
...
```
User                          MBTI Movie Archive System
  |  <---- Enter Info ---->   |
  |  -- Take Survey /      -> |  User Input Module
  |     Select Movie          |    - Name / Age / MBTI Input
                              |  Survey Module
Movie Database                |    - Preference & Mood Survey
  |  <-- Request/Response --> |  Recommendation Engine
  |                           |    - MBTI-Based Recommendation
  Movie Database              |  Movie Service Module
    - Movie Information       |    - Movie List & Detail Display
    - Genre Data              |  Reward System
    - MBTI Mapping Data       |    - Point & Reward Provision
```
